...
export const axiosData = async<T = any>(url:string) => {
...
export const axiosGet = async <T=any>(url:string):Promise<T | undefined> => {
    try{
        const reqUrl = `http://localhost:9000${url}`;
        // const reqUrl = `${url}`;
        const response = await api.get<T>(reqUrl);
        return response?.data;
    }catch(error) {
...
export const axiosPost = async<T=any>(url:string, data:any) => {
    try{
        const reqUrl = `http://localhost:9000${url}`;
        ...
        const response = await api.post<T>( reqUrl, data,
...
export const axiosDataPost = async<T=any> (url:string, data:any, customHeaders={}) => {
    try {
        const reqUrl = `http://localhost:9000${url}`;
        ...
        else if ((customHeaders as any).Cookie) {
            const cookies = (customHeaders as any).Cookie.split(';');
            const xsrfCookie = cookies.find((c:string) => c.trim().startsWith('XSRF-TOKEN='));
        ...
        const response = await api.post<T>(reqUrl, data, { headers }); // api.post 대신 axios.post 권장 (서버사이드 이슈 방지)
        return response.data;
